<script setup lang="ts">
  import { computed, inject, onBeforeMount, ref, toRef } from 'vue'
  import { isClient, useEventListener } from '@vueuse/core'
  import { useNamespace } from '@zzui/hooks'
  import { scrollbarContextKey } from '@zzui/tokens'
  import { throwError } from '@zzui/utils'
  ...
  if (!scrollbar) throwError(COMPONENT_NAME, 'can not inject scrolblar context')
  ...
  const clickTrackHandler = (e: MouseEvent) => {
    ...
    const thumbPositionPerecentage =
      ((offset - thumbHalf) * 100 * offsetRatio.value) / instance.value[bar.value.offset]

    scrollbar.wrapElement[bar.value.scroll] =
      (thumbPositionPerecentage * scrollbar.wrapElement[bar.value.scrollSize]) / 100
  }
  ...
  const mouseUpDocumentHandler = () => {
    ...
    document.removeEventListener('mousemove', mouseMoveScrollbarHandler)
    document.removeEventListener('mouseleave', mouseLeaveScrollbarHandler)
